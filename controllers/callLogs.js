...
async function checkInsertedRecord(auth, insertResponse, tenantId, status) {
  setTimeout(() => {
    let options = {
      ...
      url:
        auth.instance_url +
        "/services/data/v52.0/jobs/ingest/" +
        insertResponse.id,
      headers: {
        Authorization: auth.token_type + " " + auth.access_token,
...
async function insertCallLogs(auth, insertResponse, tenantId) {
  ...
  if (insertIds[0]) {
    await db.sequelize.query(
      "update call_logs SET sync = true, sf_insert_id = :sfInsertId where id IN (:insertIds)",
      {
        replacements: {
          insertIds: insertIds,
...
async function insertTransferLogs(auth, insertResponse, tenantId) {
  ...
  if (insertIds[0]) {
    await db.sequelize.query(
      "update transfer_logs SET sync = true, sf_insert_id = :sfInsertId where id IN (:insertIds)",
      {
        replacements: {
          insertIds: insertIds,
          sfInsertId: insertResponse.id,
  ...
  try {
    ...
    return new Promise(function (resolve, reject) {
      request(options, function (error, response) {
        if (error) throw new Error(error);
        fs.writeFile(
          "assets/csvFiles/csvFile" + tenantId + ".csv",
          response.body,
          function (err) {
            if (err) throw err;
...
async function salesForceLogin(tenantId) {
  try {
    ...
    let decodeToken = await decodedToken(salesForceUser[0].secretToken);

    // console.log(decodeToken);
    // console.log("here");
    let sendPassword = decodedPassword + decodeToken;
    var options = {
      method: "post",
...
function decodedToken(secretToken) {
  let decodedtoke = jwt.decode(secretToken, require("../config/secret.js")());
  return decodedtoke;
}
