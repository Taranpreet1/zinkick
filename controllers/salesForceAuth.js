...
let salesForceAuth = {
  addSalesForceAccount: async function (req, res) {
    try {
      ...
      let sendPassword = req.body.password + req.body.secretToken;
      console.log(sendPassword);

      let validateDetails = await salesForceLogin(
        req.body.userName,
        sendPassword,
        req.body.clientId,
      ...
      let encodeToke = await encodeToken(req.body.secret_token);

      let salesForceUser = await db.sequelize.query(
        "select * from crm_integration where tenant_id = :tenantId limit 1",
        {
      ...
      if (salesForceUser[0]) {
        let update = await db.sequelize.query(
          "update crm_integration SET username = :username,password = :password, client_id = :clientId, client_secret = :clientSecret, updated_date = :updatedDate, secret_token = :secretToken, sync_time = :syncTime, status = :status where tenant_id = :tenantId",
          {
            replacements: {
              tenantId: userCred.tenantId,
              username: req.body.userName,
              password: encodedPassword,
              secretToken: encodeToke,
      ...
      let insert = await db.sequelize.query(
        "insert into crm_integration (tenant_id, username, password, client_id, secret_token, client_secret,sync_time, created_date, status) values (:tenantId,:username, :password, :clientId, :clientSecret, :syncTime, :createdDate, :status)",
        {
          replacements: {
            tenantId: userCred.tenantId,
            username: req.body.userName,
...
function encodeToken(secretToken) {
  var sec = jwt.encode(
    {
...
function decodePassword(passwordToken) {
  let decodedPassword = jwt.decode(
    passwordToken,
